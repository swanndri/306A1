...
class Caregiver(navigation.Navigation):
	...
	def __init__(self):
		...
		self.navigate = navigation.Navigation("robot_Caregiver")
	...
	def process_event(self, action_msg):
		if ('Caregiver.caregiver_' in action_msg.data):
			self.task_list.append(action_msg.data)

	def perform_task(self, task):
		self.status = "active" 

		if ("Caregiver.caregiver_" in task):
			self.navigate.current_path = list(self.caregiver_path)
			self.navigate.target_coordinate = self.navigate.current_path.pop(0)

if __name__ == '__main__':
	rospy.init_node('caregiver_robot')
	caregiver = Caregiver()
