...
class Entertainment(navigation.Navigation):
	...
	def __init__(self):
		...
		self.navigate = navigation.Navigation("robot_4")
	...
	def process_event(self, action_msg):
		if ('entertainment.entertainment_' in action_msg.data):
			self.task_list.append(action_msg.data)

	def perform_task(self, task):
		self.status = "active" 

		if ("entertainment.entertainment_" in task):
			self.navigate.current_path = list(self.entertainment_path)
			self.navigate.target_coordinate = self.navigate.current_path.pop(0)

if __name__ == '__main__':
	rospy.init_node('entertainment_robot')
